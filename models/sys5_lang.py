...
class Model(nn.Module) :
    def __init__(self, lang_dims, nspeakers, rnn_dims, fc_dims, global_decoder_cond_dims, upsample_factors, normalize_vq=False,
        ...
        self.num_params()
        self.overtone = Overtone(rnn_dims, fc_dims, 128, 128+4)
    ...
    def forward(self, x, samples, lang):
        ...
        global_codes = torch.cat((global_spk, lang) , dim=1 )
        return self.overtone(x, code_x, global_codes), vq_pen.mean(), encoder_pen.mean(), entropy, vq_pen_spk.mean(), encoder_pen_spk.mean(), entropy_spk, spk_predict_spk, spk_predict_phn
    ...
    def generate(self, x, A, langs, deterministic=False, use_half=False, verbose=False):
        self.eval()
        with torch.no_grad() :
            ...
            discrete_spk = discrete_spk.squeeze(2)
            global_spk = discrete_spk.squeeze(1)
            global_codes = torch.cat((global_spk, langs) , dim=1 )
            output = self.overtone.generate(code_x, global_codes)
        return output


    
    def forward_validate(self, x, A, lang, deterministic=False, use_half=False, verbose=False):
        if use_half:
            A = A.half()
        self.eval()
        with torch.no_grad() :
    ...
    def tmp_func(self, batch):
        return env.collate_multispeaker_samples_lang(self.left, self.win, self.right, batch)

    def tmp_func2(self, batch):
        return env.collate_multispeaker_samples_lang_forward(self.left, self.win, self.right, batch)
    ...
    def do_train(self, paths, dataset, optimiser, epochs, batch_size, num_workers, step, train_sampler, device, lr=1e-4,  spk_lr=0.01, valid_index=[], use_half=False, do_clip=False):
        ...
        for e in range(epochs) :
            ...
            for i, (speaker, lang, wave16) in enumerate(trn_loader) :

                speaker = speaker.cuda(device)
                lang = lang.cuda(device)
                ...
                p_cf, vq_pen, encoder_pen, entropy, vq_pen_spk, encoder_pen_spk, entropy_spk, spk_predict1, spk_predict2 = self(x, translated, lang)
                ...
                # truncate this loss instead of trying to weight it
                if spk_loss2 > 10.0:
                    spk_loss2 = 10.0
...
f'-- Lc={max_loss_c:#.4} -- Lf={max_loss_f:#.4} -- Lvq={max_loss_vq:#.4} -- Lvqc={max_loss_vqc:#.4} -- Lvqcw={max_loss_vqcw:#.4} -- LvqS={max_loss_vq_spk:#.4} -- LvqcS={max_loss_vqc_spk:#.4} -- LvqcwS={max_loss_vqcw_spk:#.4}')
    ...
    def validate(self, paths, step, data_path, test_index, device, deterministic=False, use_half=False, verbose=False):
        ...
        batch_size = 10
        dataset = env.MultispeakerDataset_lang(test_index, data_path)
        ...
        for i, (speaker, lang, wave16, path) in enumerate(test_loader) :
            wave_write = wave16
            wave16 = wave16.cuda()
            ...
            vq_pen, encoder_pen, entropy, vq_pen_spk, encoder_pen_spk, entropy_spk, spk_predict1, spk_predict2 = self.forward_validate(samples, translated, lang)
            running_loss_c += 0
            running_loss_f += 0
